...
class Policy(nn.Module):
    def __init__(self, obs_shape, action_space, base=None, base_kwargs=None):
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            if self.args.distribution == 'beta':
                self.dist = DiagBeta(self.base.output_size, num_outputs)

            if self.args.distribution == 'UnitSTD':
                self.dist = DiagGaussUnitSTD(self.base.output_size, num_outputs)
    ...
    def act(self, inputs, rnn_hxs, masks, deterministic=False):
        ...
        dist, p1, p2 = self.dist(actor_features)
    ...
    def evaluate_actions(self, inputs, rnn_hxs, masks, action):
        value, actor_features, rnn_hxs = self.base(inputs, rnn_hxs, masks)
        dist, alpha, beta = self.dist(actor_features)
        action_log_probs = dist.log_probs(action)
        dist_entropy = dist.entropy().mean()
